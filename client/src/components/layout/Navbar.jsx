...
const Navbar = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-[#030303]/90 backdrop-blur-2xl border-b border-white/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <Link to={isAuthenticated ? getDashboardLink() : '/'} className="flex items-center gap-3 group">
            ...
            <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-[#a078ff] to-[#6d3bd7] p-[1px]">
              <div className="w-full h-full bg-[#0e0e0e] rounded-xl flex items-center justify-center group-hover:bg-transparent transition duration-300">
                <FiHexagon className="text-[#d0bcff] group-hover:text-white text-lg" />
              </div>
            </div>
        ...
        <div className="hidden md:flex items-center gap-6">
          <Link
            ...
            className={`text-xs font-mono-code uppercase tracking-wider transition ${
              location.pathname === '/hackathons' ? 'text-[#d0bcff] font-bold border-b border-[#a078ff] pb-1' : 'text-gray-400 hover:text-white'
            }`}
          >
            Explore
          </Link>
          ...
          {isAuthenticated && (
            <Link
              to={getDashboardLink()}
              className={`text-xs font-mono-code uppercase tracking-wider transition ${
                location.pathname.includes('/dashboard') ? 'text-[#d0bcff] font-bold border-b border-[#a078ff] pb-1' : 'text-gray-400 hover:text-white'
              }`}
            >
              Dashboard
            </Link>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link to="/profile" className="flex items-center gap-2.5 p-1.5 pl-3 rounded-full bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 transition">
                <div className="flex flex-col text-right">
                  ...
                  <span className="text-[9px] font-mono-code text-[#d0bcff] uppercase tracking-wider">{user.role}</span>
                </div>
                <div className="w-8 h-8 rounded-full bg-[#a078ff]/20 border border-[#a078ff]/40 flex items-center justify-center font-bold text-xs text-[#d0bcff] uppercase">
      ...
      {/* Mobile Drawer */}
      {mobileOpen && (
        <div className="md:hidden bg-[#0e0e0e] border-b border-white/10 px-4 py-5 space-y-3 font-mono-code text-xs uppercase">
          ...
          {isAuthenticated ? (
            <>
              <Link to={getDashboardLink()} onClick={() => setMobileOpen(false)} className="block text-[#d0bcff] font-bold py-1.5">Dashboard</Link>
